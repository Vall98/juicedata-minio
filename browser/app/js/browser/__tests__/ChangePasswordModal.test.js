...
describe("ChangePasswordModal", () => {
  ...
  it("should not generate accessKey for IAM User", () => {
    ...
    return new Promise(resolve => setTimeout(() => {
      expect(screen.getByDisplayValue("minio")).toBeTruthy()
      expect(screen.getByDisplayValue("rsecretkey")).toBeTruthy()
      resolve()
    }, 0))
  })
  ...
  it("should not show new accessKey field for IAM User", () => {
    render(<ChangePasswordModal serverInfo={serverInfo} />)
    expect(screen.getByLabelText("Current Access Key")).toBeTruthy()
    expect(screen.getByLabelText("Current Secret Key")).toBeTruthy()
    expect(screen.getByLabelText("New Secret Key")).toBeTruthy()
    expect(screen.queryByLabelText(/New Access Key/i)).toBeNull()
  })
